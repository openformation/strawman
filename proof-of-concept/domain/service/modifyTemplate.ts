...
export const makeModifyTemplate =
  ...
  (
    ...
    const templateParentIsNew = newNodes.has(node);

    newNodes.delete(node);
    ancestors.pop();
    node = node.withAddedTemplate(anHTTPMethod, aTemplate);

    if (templateParentIsNew) newNodes.add(node);
    ancestors.push(node);
